...
class Matrix extends React.Component {
  ...
  isResizeableX() {
    ...
    return (
      !this.props.readonly &&
      (resize === 'horizontal' || resize === 'both' || resize === undefined)
    );
  }
  ...
  isResizeableY() {
    ...
    return (
      !this.props.readonly &&
      (resize === 'vertical' || resize === 'both' || resize === undefined)
    );
  }
  ...
  moveCell(dx, dy) {
    ...
    if (
      this.state.caret + dx >
      ('' + this.getCellValue(cellX, this.state.y)).length
    ) {
      cellX++;
    ...
    if (!this.isResizeableY() && cellY >= this.getHeight())
      cellY = this.state.y;
